...
class DroneTrainer:
    ...
    def default_settings(self):
        # Default settings tailored for FlyThruGoalGateAviary
        ...
        self.max_training_timesteps = int(1000000)
        self.ppo_epochs = 800
        self.eps_clip = 0.2
        self.gamma = 0.99
    ...
    def initialize_agent(self):
        # Check and set device to GPU if available
        ...
        self.agent.policy.to(self.device)  # Move the PPO policy model to GPU

    def setup_logging(self):
        if not os.path.exists(self.checkpoint_base):
            os.makedirs(self.checkpoint_base)
        self.log_file_path = os.path.join(self.log_dir, f"Train_GoalGate_LOG.csv")
    ...
    def run_episode(self):
        obs, info = self.env.reset(seed=self.random_seed, options={})
        current_ep_reward = 0
    ...
    def handle_timestep_updates(self):
        ...
        if self.time_step % self.print_freq == 0:
            ...
            if avg_reward > self.best_reward:
                ...
                best_model_path = os.path.join(self.checkpoint_base, "best_goal.pth")
    ...
    def save_model(self):
        checkpoint_path = os.path.join(self.checkpoint_base, f"{self.i_episode}_checkpoint_goal.pth")
