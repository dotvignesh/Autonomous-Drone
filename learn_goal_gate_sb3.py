...
def make_env(rank):
    def _init():
        env = FlyThruGoalGateAviary(obs=ObservationType("kin"), act=ActionType("rpm"))
        return Monitor(env)
    return _init
...
def train_gate():
    ...
    log_file_path = os.path.join(log_dir, "Train_GoalGate_sb3_LOG.csv")
    num_envs = 8
    ...
    model = PPO(
        ...
        ent_coef=0.01,
        verbose=1,
        tensorboard_log=os.path.join("log_dir/PPO_goal_gate_final_SB3/", "tensorboard/"),
        seed=0
    )

    callback = LoggingCallback(log_path=log_file_path, verbose=1)
    model.learn(total_timesteps=int(200000), callback=callback)
    model_path = os.path.join("log_dir/PPO_goal_gate_final_SB3/", "ppo_goal_gate_final")
