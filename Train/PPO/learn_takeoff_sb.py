...
def make_env(rank):
    def _init():
        env = TakeoffAviary(obs=ObservationType("kin"), act=ActionType("rpm"))
        return Monitor(env)
    return _init
...
def train_takeoff():
    ...
    log_dir = "sb3_takeoff_logs/"
    os.makedirs(log_dir, exist_ok=True)
    log_file_path = os.path.join(log_dir, "Train_Takeoff_LOG.csv")
    ...
    model_path = os.path.join(log_dir, "ppo_takeoff_final")
